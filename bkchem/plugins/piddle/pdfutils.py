...
def cacheImageFile(filename):
    ...
    f.write(LINEEND.join(code).encode('ascii') + LINEEND.encode('ascii'))
    f.close()
...
def _AsciiHexEncode(input):
    ...
    output = BytesIO()
    for char in input:
        output.write(('%02x' % char).encode('ascii'))
    output.write(b'>')
    output.seek(0)
    return output.read()
...
def _AsciiHexDecode(input):
    ...
    while i < len(stripped):
        ...
        output.write(bytes([int(twobytes, 16)]))
        i = i + 2
    output.seek(0)
    return output.read()
...
def _AsciiHexTest(text='What is the average velocity of a sparrow?'):
    ...
    for i in range(whole_word_count):
        ...
        if num == 0:
            ...
            outstream.write(bytes([c1+33, c2+33, c3+33, c4+33, c5+33]))
    ...
    if remainder_size > 0:
        ...
        lastword = [c1+33, c2+33, c3+33, c4+33, c5+33]
        #write out most of the bytes.
        outstream.write(bytes(lastword[0:remainder_size + 1]))
...
def _AsciiBase85Decode(input):
    ...
    for i in range(whole_word_count):
        ...
        outstream.write(bytes([b1, b2, b3, b4]))
    ...
    if remainder_size > 0:
        ...
        if remainder_size == 2:
            ...
            lastword = [b1, b2, b3+1]
        outstream.write(bytes(lastword))
...
def _wrap(input, columns=60):
    ...
    if isinstance(input, bytes):
        return (LINEEND.encode('ascii')).join(output)
    else:
