...
class PDFDocument:
    ...
    def writeXref(self, f):
        ...
        f.write(('xref' + LINEEND).encode('latin1'))
        f.write(('0 %d' % (len(self.objects) + 1) + LINEEND).encode('latin1'))
        f.write(('0000000000 65535 f' + LINEEND).encode('latin1'))
        for pos in self.xref:
            f.write(('%0.10d 00000 n' % pos + LINEEND).encode('latin1'))
    ...
    def writeTrailer(self, f):
        f.write(('trailer' + LINEEND).encode('latin1'))
        f.write(('<< /Size %d /Root %d 0 R /Info %d 0 R>>' % (len(self.objects) + 1, 1, self.infopos)  + LINEEND).encode('latin1'))
        f.write(('startxref' + LINEEND).encode('latin1'))
        f.write((str(self.startxref)  + LINEEND).encode('latin1'))
    ...
    def SaveToFileObject(self, fileobj):
        ...
        i = 1
        self.xref = []
        f.write(("%PDF-1.2" + LINEEND).encode('latin1'))  # for CID support
        f.write(("%" + chr(0xAA) + chr(0xAB) + chr(0xAC) + chr(0xAD) + LINEEND).encode('latin1'))
        for obj in self.objects:
            pos = f.tell()
            self.xref.append(pos)
            f.write((str(i) + ' 0 obj' + LINEEND).encode('latin1'))
            obj.save(f)
            f.write(('endobj' + LINEEND).encode('latin1'))
            i = i + 1
        self.writeXref(f)
        self.writeTrailer(f)
        f.write(b'%%EOF')  # no lineend needed on this one!
...
class PDFObject:
    ...
    def save(self, file):
        ...
        file.write(('% base PDF object' + LINEEND).encode('latin1'))
    def printPDF(self):
...
class PDFLiteral(PDFObject):
    ...
    def __init__(self, text):
        self.text = text
    def save(self, file):
        file.write((self.text + LINEEND).encode('latin1'))
...
class PDFCatalog(PDFObject):
    ...
    def save(self, file):
        file.write((self.template % (self.RefPages, self.RefOutlines) + LINEEND).encode('latin1'))
...
class PDFOutline(PDFObject):
    ...
    def __init__(self):
        self.template = LINEEND.join([
                ...
                '>>'])
    def save(self, file):
        file.write((self.template + LINEEND).encode('latin1'))
...
class PDFPageCollection(PDFObject):
    ...
    def save(self, file):
        ...
        file.write((text + LINEEND).encode('latin1'))
...
class PDFPage(PDFObject):
    ...
    def save(self, file):
        ...
        file.write((self.template % self.info + LINEEND).encode('latin1'))
...
class PDFStream(PDFObject):
    ...
    def save(self, file):
        ...
        if self.compression:
            file.write(('<< /Length %d /Filter [/ASCII85Decode /FlateDecode]>>' % length + LINEEND).encode('latin1'))
        else:
            file.write(('<< /Length %d >>' % length + LINEEND).encode('latin1'))
        file.write(('stream' + LINEEND).encode('latin1'))
        if isinstance(data_to_write, str):
            data_to_write = data_to_write.encode('latin1')
        file.write(data_to_write + LINEEND.encode('latin1'))
        file.write(('endstream' + LINEEND).encode('latin1'))
...
class PDFType1Font(PDFObject):
    def __init__(self, key, font):
        ...
        self.template = LINEEND.join([
                    ...
                    '>>'])
    def save(self, file):
        file.write((self.template % (self.keyname, self.fontname) + LINEEND).encode('latin1'))
